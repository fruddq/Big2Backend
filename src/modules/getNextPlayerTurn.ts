...
export const getNextPlayerTurn = (players: PlayerExtension) => {
  ...
  if (!currentPlayer) {
    return Object.keys(players).find((key) => players[key]!.roundPass === false && players[key]!.won === false)
  }
  ...
  while (nextPlayer && (players[nextPlayer]?.roundPass || players[nextPlayer]?.won)) {
